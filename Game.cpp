...

{
	...
	// Init fonts
	this->font.loadFromFile("Fonts/FC Iconic Bold Italic.otf");
	...
	// Init BGM
	this->background.openFromFile("Audio/maingame.ogg");
	...
	this->creditsPic.loadFromFile("Textures/Highscore.png");
	...
	// Init Textbox
	textbox.Setup(1, 18, 600, sf::Vector2f(1300, 0));

	// Init boss
	this->generateBoss = false;
	this->bossTex.loadFromFile("Textures/Boss.png");
}
			...
			case Keyboard::Return:
				if (gameIsOver && !isLOGO)
				{
					...
					case 0: // New game
						...
						isMenu = false;
						textbox.Clear();
						textbox.Add("\t\t\tTHANAKORN WIHOKKUN 63010255\n\t\t\t\t\t\t\tPresent\n\t\t\t\tProgramming Fundamental\nKing Mongkut's Institute of Technology Ladkrabang", true);
						welcome.play();
						break;
					case 1: // 2P mode
						ResetPlayer(playerA);
...
void Game::TwoPlayerScoreUpdate(Player& playerA, Player& playerB)
{
	...
	{
		gameIsOver = true;
		this->twoPText.setString("Player 2 WIN!!");
	}
	else if (playerB.HP <= 0)
	{
		gameIsOver = true;
		this->twoPText.setString("Player 1 WIN!!");
	}
}
